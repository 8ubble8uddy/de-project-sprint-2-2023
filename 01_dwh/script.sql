/*
Скрипт загрузки данных из источников в хранилище.
*/

/* Создание временной таблицы "tmp_sources" с данными из всех источников. */
...
/* Создание временной таблицы "tmp_sources_fact" с данными из источников "tmp_sources" и таблиц измерений "dwh.*". */
...
JOIN dwh.d_product dp ON dp.product_name = src.product_name AND
                         dp.product_description = src.product_description AND
...
ON f.product_id = t.product_id AND
   f.craftsman_id = t.craftsman_id AND
   f.customer_id = t.customer_id AND
   f.order_created_date = t.order_created_date 
WHEN MATCHED THEN
